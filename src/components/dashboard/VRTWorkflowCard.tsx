...
export default function VRTWorkflowCard({ dealId, vehicleId, stage, currentVrt, currentReg, onUpdate }: VRTWorkflowCardProps) {
  ...
  return (
    <div className={`bg-white p-8 rounded-3xl border border-slate-200 relative overflow-hidden transition-all duration-500 shadow-sm`}>
      ...
      {!isUnlocked && !isClosed && (
        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-[4px] z-20 flex items-center justify-center p-8 text-center">
      ...
      <div className={`space-y-6 ${isClosed ? 'opacity-40 pointer-events-none' : ''}`}>
        ...
        {!isRegistered && isUnlocked && (
          <button 
            ...
            className="w-full py-5 rounded-2xl bg-teal-600 text-white text-[10px] font-black uppercase tracking-[0.4em] hover:bg-teal-700 transition-all flex items-center justify-center gap-3 shadow-xl shadow-teal-100 active:scale-[0.98]"
          >
            {loading ? (
        ...
        {isRegistered && (
          <div className="p-6 rounded-2xl bg-teal-50 border border-teal-100 flex items-center gap-4">
